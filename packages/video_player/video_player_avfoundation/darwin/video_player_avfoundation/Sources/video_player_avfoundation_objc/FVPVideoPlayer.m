...
#pragma clang diagnostic pop
  ...
  // video streams (not just iOS 16). An invisible AVPlayerLayer is used to overwrite the
  // protection of pixel buffers in those streams for issue #1, and restore the correct width and
  // height for issue #2. It is also used to start picture in picture.
  ...
  _pipPlayerLayer.opacity = 0.001;
  UIViewController *vc = viewProvider.viewController;
  if (vc) {
    [vc.view.layer addSublayer:_pipPlayerLayer];
  }
...
- (void)setupPipController {
  if ([AVPictureInPictureController isPictureInPictureSupported]) {
    _pictureInPictureController =
        [[AVPictureInPictureController alloc] initWithPlayerLayer:_pipPlayerLayer];
...
- (void)startOrStopPictureInPicture:(BOOL)shouldPictureInPictureStart {
  if (![AVPictureInPictureController isPictureInPictureSupported] ||
      _isPictureInPictureStarted == shouldPictureInPictureStart) {
    return;
  }

  _isPictureInPictureStarted = shouldPictureInPictureStart;
  if (_pictureInPictureController && _isPictureInPictureStarted &&
      ![_pictureInPictureController isPictureInPictureActive]) {
    if (self.eventListener != nil) {
      // The event is already sent here to make sure that Flutter UI can be updated as soon as
      // possible.
      [self.eventListener videoPlayerDidStartPictureInPicture];
    }
    [_pictureInPictureController startPictureInPicture];
  } else if (_pictureInPictureController && !_isPictureInPictureStarted &&
             [_pictureInPictureController isPictureInPictureActive]) {
...
#if TARGET_OS_IOS
- (void)setAutomaticallyStartPictureInPicture:(BOOL)enable
                                        error:(FlutterError *_Nullable *_Nonnull)error {
  if (@available(iOS 14.2, *)) {
    _pictureInPictureController.canStartPictureInPictureAutomaticallyFromInline = enable;
  }
}

- (void)setPictureInPictureOverlayRect:(FVPPictureInPictureOverlayRect *)rect
                                 error:(FlutterError *_Nullable *_Nonnull)error {
  if (_pipPlayerLayer) {
    _pipPlayerLayer.frame = CGRectMake(rect.left, rect.top, rect.width, rect.height);
  }
}
...
- (void)startPictureInPicture:(FlutterError *_Nullable *_Nonnull)error {
  ...
  if (![backgroundModes isKindOfClass:[NSArray class]] ||
    ...
    *error = [FlutterError errorWithCode:@"video_player"
                                 message:@"UIBackgroundModes audio is missing in Info.plist"
                                 details:nil];
    return;
  }
...
#else
// macOS stubs — PiP is not supported on macOS.
- (void)setAutomaticallyStartPictureInPicture:(BOOL)enable
                                        error:(FlutterError *_Nullable *_Nonnull)error {}
- (void)setPictureInPictureOverlayRect:(FVPPictureInPictureOverlayRect *)rect
                                 error:(FlutterError *_Nullable *_Nonnull)error {}
